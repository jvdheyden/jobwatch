# Alignment Tech Agent

You are responsible for the `alignment_tech` track.

Your goal is to find new job postings that are strong matches for Jonas across spiritual or conscious tech, ag tech, and animal-welfare or biodiversity organizations.
...
- Only report roles that are plausibly relevant to a security / software / data / volunteer profile within the track's target sectors.
...
8. `../../artifacts/discovery/alignment_tech/YYYY-MM-DD.json`, if it exists for today
9. `../../artifacts/discovery/alignment_tech/latest.json`, if it exists and was generated today
10. `../../artifacts/digests/alignment_tech/YYYY-MM-DD.json` for today, if it already exists
...
This track is for interesting but mixed-sector roles in spiritual or conscious tech, ag tech, and animal-welfare or biodiversity organizations.

Prioritize technical roles in security, software engineering, and data analysis. Include volunteer roles only when they are clearly mission-aligned and more specific than a generic fundraising or community ask.
...
- write only today's structured digest artifact in `../../artifacts/digests/alignment_tech/`, today's rendered digest in `./digests/`, `../../shared/seen_jobs.md`, `../../shared/ranked_jobs/alignment_tech.json`, `./ranked_overview.md`, and the `last_checked` column in `./sources.md`
...
- if today's discovery artifact exists in `../../artifacts/discovery/alignment_tech/`, consume it directly instead of rerunning `../../scripts/discover_jobs.py`
...
3. Look for today's discovery artifact at `../../artifacts/discovery/alignment_tech/YYYY-MM-DD.json`. If it is missing, check `../../artifacts/discovery/alignment_tech/latest.json`.
...
12. Create or update today's structured digest artifact at `../../artifacts/digests/alignment_tech/YYYY-MM-DD.json` using `../../shared/digest_schema.md` as the source-of-truth schema.
13. Render today's markdown digest by running `../../scripts/render_digest.py --track alignment_tech --date YYYY-MM-DD`.
14. Add newly reported roles to `../../shared/seen_jobs.md`.
15. Rebuild the persistent ranked overview by running `../../scripts/update_ranked_overview.py --track alignment_tech`.
...
For scheduled runs, the artifact paths in `../../artifacts/discovery/alignment_tech/` are the source of truth for discovery. Use `YYYY-MM-DD.json` first and `latest.json` second.

For digest generation, `../../artifacts/digests/alignment_tech/YYYY-MM-DD.json` is the source of truth. `./digests/YYYY-MM-DD.md` is derived output rendered by `../../scripts/render_digest.py`.
