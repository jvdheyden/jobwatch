...
You are responsible for the `public_service` track.

Your goal is to find new job postings that are strong matches for Jonas in public-interest technical roles across government, NGOs, standards bodies, defense industry, and digital public infrastructure.
...
- Only report roles that are plausibly relevant to a cryptography / cybersecurity / public-interest technical or advisory profile.
...
8. `../../artifacts/discovery/public_service/YYYY-MM-DD.json`, if it exists for today
9. `../../artifacts/discovery/public_service/latest.json`, if it exists and was generated today
10. `../../artifacts/digests/public_service/YYYY-MM-DD.json` for today, if it already exists
...
This track is for public-interest technical roles with strong relevance to cryptography, cybersecurity, privacy, defensive security, technical advisory work, or adjacent public-service security functions.

Treat German and English role-title variants as equivalent when judging fit.
...
- write only today's structured digest artifact in `../../artifacts/digests/public_service/`, today's rendered digest in `./digests/`, `../../shared/seen_jobs.md`, `../../shared/ranked_jobs/public_service.json`, `./ranked_overview.md`, and the `last_checked` column in `./sources.md`
...
- if today's discovery artifact exists in `../../artifacts/discovery/public_service/`, consume it directly instead of rerunning `../../scripts/discover_jobs.py`
...
3. Look for today's discovery artifact at `../../artifacts/discovery/public_service/YYYY-MM-DD.json`. If it is missing, check `../../artifacts/discovery/public_service/latest.json`.
...
12. Create or update today's structured digest artifact at `../../artifacts/digests/public_service/YYYY-MM-DD.json` using `../../shared/digest_schema.md` as the source-of-truth schema.
13. Render today's markdown digest by running `../../scripts/render_digest.py --track public_service --date YYYY-MM-DD`.
14. Add newly reported roles to `../../shared/seen_jobs.md`.
15. Rebuild the persistent ranked overview by running `../../scripts/update_ranked_overview.py --track public_service`.
...
For scheduled runs, the artifact paths in `../../artifacts/discovery/public_service/` are the source of truth for discovery. Use `YYYY-MM-DD.json` first and `latest.json` second.

For digest generation, `../../artifacts/digests/public_service/YYYY-MM-DD.json` is the source of truth. `./digests/YYYY-MM-DD.md` is derived output rendered by `../../scripts/render_digest.py`.
